...
for fcount=10:10 %for each point cloud
...
%     if cratio1 > 0.7 & cratio2 > 0.7 & size(pts1,1) > 5 &  size(pts2,1) > 5 
    if size(pts1,1) > 5 
                fprintf('i is %.0f \n', i);

        if i == 7
            disp('got 6')
            [angle1, diameter1] = branch_angle_diameter(pts1,1);
        end
         [angle1, diameter1] = branch_angle_diameter(pts1,0);
         if diameter1 > 1.5
%              pts = pts1;
             ...
             pts = pts1(bidx,:);
             [angle1, diameter1] = branch_angle_diameter(pts,0);
         end
