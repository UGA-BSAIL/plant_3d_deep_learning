...
def evaluate(configs=None):
    ...
    for shape_idx, file_path in enumerate(tqdm(dataset.file_paths, desc='eval', ncols=0)):
        ...
        act_coords = np.array([], dtype=np.float32).reshape(3,0)
        act_gt = np.array([], dtype=np.int64).reshape(0,)
        act_pd = np.array([], dtype=np.int64).reshape(0,)
        for shape_index, voxel in enumerate(tqdm(voxels, desc='eval', ncols=0)):
