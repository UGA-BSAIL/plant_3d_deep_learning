...
def evaluate(configs=None):
    ...
    for shape_index, voxel in enumerate(tqdm(dataset.voxels, desc='eval', ncols=0)):
        ...
        output_color_point_cloud(coords, ground_truth, os.path.join(output_dir, str(shape_index)+'_gt.txt'))
        output_color_point_cloud(coords, predictions, os.path.join(output_dir, str(shape_index)+'_pred.txt'))
        output_color_point_cloud_red_blue(coords, np.int32(ground_truth == predictions), os.path.join(output_dir, str(shape_index)+'_diff.txt'))
        #shape_index+=1
