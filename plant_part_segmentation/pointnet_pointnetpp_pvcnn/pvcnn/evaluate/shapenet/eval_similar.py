...
def evaluate(configs=None):
    ...
    for shape_index, (file_path) in enumerate(tqdm(dataset.file_paths, desc='eval', ncols=0)):
        ...
        torch.cuda.reset_max_memory_allocated(configs.device)
        #print("1st step: \n")
        #print(torch.cuda.memory_summary(configs.device))
        ...
        print('torch.cuda.max_memory_allocated()', torch.cuda.max_memory_allocated(configs.device))
